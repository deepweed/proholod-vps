...
import CommercialSplit from '@/components/CommercialSplit/CommercialSplit';
...
async function getProducts(page: number): Promise<{ products: Product[], currentPage: number, totalPages: number }> {
  ...
  const products = await prisma.product.findMany({
    where: {
      category: 'commercial-split-systems',
    },
    include: {
      images: true,
      stats: true,
...
export default async function CommercialSplitPage({ searchParams }: { searchParams: { page?: string } }) {
  const page = parseInt(searchParams.page || '1', 10);
  const { products, currentPage, totalPages } = await getProducts(page);

  return <CommercialSplit products={products} currentPage={currentPage} totalPages={totalPages} />;
}
